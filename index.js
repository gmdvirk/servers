...
const dbConfig = require('./mysql_connections.json');
const descordConfig = require('./descordconfig.json');
 ...
 async function checkserverstatus() {
    ...
    for(let i=0;i<userData.data.length;i++){
      ...
      if(tempindex!==-1){
        ...
        // info.serverlist.forEach(async(obj,index)=>{
        //   let result= await processServerEvent(data,obj)
        //  if(result.length===0){
        //     info.serverlist[index].check="no response"
        //     allmeassages.push({...descordConfig,message:`Host ip ${info.serverlist[index].ipv4[0]} and  Label : ${info.serverlist[index].label} didn't respond to test packet.`})
        //     if(!((isWithin60Minutes(info.serverlist[index].laststartup,getCurrentEpochTimestamp())))){
        //       info.serverlist[index].laststartup=getCurrentEpochTimestamp()
        //       reboot({hostId:info.serverlist[index].ipv4[0],linodeId:info.serverlist[index].id})  
        //     }
        //   }else{
        //     info.serverlist[index].check="success"
        //     allmeassages.push({...descordConfig,message:`Host ip ${info.serverlist[index].ipv4[0]} and  Label : ${info.serverlist[index].label} responded successfully to test packet.`})
        //   }
        // })
